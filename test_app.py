...
def test_get_single_item_not_found(client):
    """Fetching an item ID that doesn't exist should 404."""
...
def test_patch_item_not_found(client):
    """Patching an item ID that doesn't exist should 404."""
    res = client.patch('/inventory/999', json={"price": 10.0})
    assert res.status_code == 404


def test_delete_item_not_found(client):
    """Deleting an item ID that doesn't exist should 404."""
...
def test_external_registry_barcode_not_found(mock_get, client):
    """OpenFoodFacts returning status 0 means the barcode wasn't found."""
    mock_get.return_value.status_code = 200
    mock_get.return_value.json.return_value = {
        "status": 0,  # 0 means the barcode isn't in their system
